...
namespace :db do
	...
	task :copy_from_production => :environment do
		...
		`mysqldump --add-drop-table --single-transaction --allow-keywords --hex-blob --quick -u #{db_config['production']['username']} -p#{db_config['production']['password']} -h #{db_config['production']['host']} --set-variable=max_allowed_packet=100M #{db_config['production']['database']} > #{temp_file.path}`
