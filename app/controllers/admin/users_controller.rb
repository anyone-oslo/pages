class Admin::UsersController < Admin::AdminController
	...
	def new_password
		if params[:username]
			if user = User.find_by_username_or_email( params[:username] )
				...
				user.save
				AdminMailer.deliver_new_password( :user => user, :site_name => Backstage.config( :site_name ), :password => new_password, :login_url => url_for( :controller => 'admin/pages', :action => :index, :only_path => false ) )
	...
	def create
		...
		if @user.save
			AdminMailer.deliver_new_user( :user => @user, :site_name => Backstage.config( :site_name ), :login_url => url_for( :controller => 'admin/pages', :action => :index, :only_path => false ) )
